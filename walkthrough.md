...
1. **Simplified & Cleaner Workbench Layout**:
...
2. **Outlet Orifice Diameter ($d$) Can Be 0 ($d = 0$)**:
...
3. **Lower Limits for Crest and Spillway Heights**:
...
4. **Removed "Figure 2.2 Formulations" Badges**:
...
5. **Spillway Weir Length ($L_{\text{savak}}$) Starts from 0**:
...
6. **Replaced Constant Pool Area with Realistic Maximum Storage Capacity ($S_{\text{maks}}$)**:
...
- Changes committed and pushed to `feature/playground-hero` on GitHub (`394115d`).
