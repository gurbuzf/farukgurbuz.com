...
1. **Outlet Orifice Diameter ($d$) Can Be 0 ($d = 0$)**:
   - The slider now allows `min = 0` (range 0 to 2.8 m).
...
2. **Lower Limits for Crest and Spillway Heights**:
...
3. **Removed "Figure 2.2 Formulations" Badges**:
...
4. **Spillway Weir Length ($L_{\text{savak}}$) Starts from 0**:
...
5. **Replaced Constant Pool Area with Realistic Maximum Storage Capacity ($S_{\text{maks}}$)**:
...
- Changes committed and pushed to `feature/playground-hero` on GitHub (`a9ad065`).
